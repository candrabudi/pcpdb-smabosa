<div class="card mb-4">
    ...
    <hr class="my-0" />
    <div class="card-body">
        <form id="formAccountSettings" method="POST" action="{{ route($route) }}">
            @csrf
            <div class="row">
                <div class="mb-3 col-md-6">
                    <label for="{{ $parentType }}_parent_name" class="form-label">Nama {{ $title }}</label>
                    <input class="form-control" type="text" id="{{ $parentType }}_parent_name" name="{{ $parentType }}_parent_name" value="{{ $parent->parent_name ?? '' }}" autofocus />
                </div>

                <div class="mb-3 col-md-6">
                    <label for="{{ $parentType }}_birth_place" class="form-label">Tempat Lahir</label>
                    <input class="form-control" type="text" id="{{ $parentType }}_birth_place" name="{{ $parentType }}_birth_place" value="{{ $parent->birth_place ?? '' }}" autofocus />
                </div>

                <div class="mb-3 col-md-6">
                    <label for="{{ $parentType }}_birth_date" class="form-label">Tanggal Lahir</label>
                    <input type="text" class="form-control" name="{{ $parentType }}_birth_date" value="{{ $parent->birth_date ?? '' }}" placeholder="YYYY-MM-DD" id="flatpickr-date-{{ $parentType }}" />
                </div>
                <div class="mb-3 col-md-6">
                ...
                <div class="mb-3 col-md-6">
                    ...
                    <input class="form-control" type="text" id="{{ $parentType }}_profession" name="{{ $parentType }}_profession" value="{{$parent->profession ?? ''}}" autofocus />
                </div>
                <div class="mb-3 col-md-6">
                    <label for="{{ $parentType }}_income" class="form-label">Pendapatan</label>
                    <input class="form-control" type="text" id="{{ $parentType }}_income" name="{{ $parentType }}_income" value="{{$parent->income ?? ''}}" autofocus />
                </div>
                <div class="mb-3 col-md-6">
                    <label class="form-label" for="{{ $parentType }}_whatsapp_phone">Nomor Whatsapp</label>
                    <div class="input-group input-group-merge">
                        <span class="input-group-text"><i class="ti ti-phone"></i></span>
                        <input type="text" id="{{ $parentType }}_whatsapp_phone" name="{{ $parentType }}_whatsapp_phone" class="form-control" value="{{$parent->whatsapp_phone ?? ''}}" />
                    </div>
                </div>
            </div>
            <div class="mt-2">
